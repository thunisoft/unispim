...
struct ServerPackageStruct
{
    bool has_update;
    QString newVersionNum;
    QString setupid;
    QString packageMd5;
    QString download_type;
    QString download_url;
    QString pacakge_name;
};
...
public:
    ...
    //请求版本更新
    int ClientVersionDetection(QString currentVersionNum, ServerPackageStruct& package_info);
    ...
    /**@brief 获得全量系统词库的地址
   * @param[in]  wordlib_md5 当前词库的md5值
   * @param[out]  wordlibid 服务器上全量词库的地址
   * @return  函数执行结果
   * - 0   查询成功
   * - other  查询失败
   */
   ...
   */
    ...
    */
    ...
    /**@brief 词库增量碎片的下载
    * @param[in]  version 当前输入法的版本
    * @param[in]  clientid 当前的用户ID
    * @param[in]  loginid 当前用户的登录id，没有登录为""
    * @param[in]  block_vector 词库碎片信息对应的容器
    * @return  函数执行结果
    * - 0  请求成功
    * - 666 请求超时
    * - Others  其他错误
    */
    int  DownloadSysWordlibBlockFromServer(QString version, QString clientid, QString loginid, QVector<WORDLIB_BLOCK_INFO> block_vector);
    ...
    int UploadHistoryRecordInfo(QMap<QString,int> history_data_map, QString clientid, QString loginid, QString version);
...
private:
    ...
    int AddExtraPramer(QByteArray& inputByteArray);
    int AddExtraPramer(QHttpMultiPart* multiPart);
