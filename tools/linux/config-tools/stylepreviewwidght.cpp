...
void StylePreviewWidght::LoadConfig()
{
    ...
    for(QString key : colorItemKeyList)
    {
        int key_value;
    ...
    int key_value;
    if(ConfigManager::Instance()->GetIntConfigItem("font_height",key_value))
    {
         m_previewModel->SetFontSize(key_value);
    }

    QString key_value_str;
    if(ConfigManager::Instance()->GetStrConfigItem("chinese_font_name",key_value_str))
    {
        m_previewModel->SetChineseFont(QFont(key_value_str));
    }
    if(ConfigManager::Instance()->GetStrConfigItem("english_font_name",key_value_str))
    {
        m_previewModel->SetEnglishFont(QFont(key_value_str));
    }

    m_previewModel->SetCandidateLayout((CANDIDATE_LAYOUT)0);

    if(ConfigManager::Instance()->GetIntConfigItem("candidates_per_line",key_value))
    {
         m_previewModel->SetCandidatesSize(key_value);
    }
    if(ConfigManager::Instance()->GetIntConfigItem("tool_bar_config",key_value))
    {
         m_previewModel->SetToolBarConfig(key_value);
    }

    m_previewModel->SetKeepExpand(false);
