...
void CancelSystemTray(TrayWindow* tray)
{
    TraySendOpcode(tray, SYSTEM_TRAY_CANCEL_MESSAGE, tray->window, 0, 0);
    //XMapWindow(dpy, tray->window);
    tray->bTrayMapped = False;
}
...
void TraySendOpcode(TrayWindow* tray,
                    ...
                    /*tray_request_dock   tray->window 0    0      */
{
    Display* dpy = tray->owner->dpy;
    XEvent ev;
...
//                 iTrappedErrorCode);
        //qDebug() << "X error " << iTrappedErrorCode << " on opcode send!";
        std::cout<< "X error " << iTrappedErrorCode << " on opcode send!";
    }
}
